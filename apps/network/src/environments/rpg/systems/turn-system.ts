...
import type { TurnManager } from '../interfaces'

export type TurnSystemOptions = {
  now?: () => number
}
...
export function normalizeTurnState(game: RpgGameState, options: TurnSystemOptions = {}): boolean {
...
export function advanceTurn(game: RpgGameState, options: TurnSystemOptions = {}): void {
  const now = options.now ?? Date.now
  game.party ??= []
  game.log ??= []
  game.round ??= 1
...
export function recomputeTurnOrder(game: RpgGameState, options: TurnSystemOptions = {}): void {
  normalizeTurnState(game, options)
}

export const turnSystem: TurnManager = {
  advance: (game) => advanceTurn(game),
  normalize: (game) => normalizeTurnState(game),
  computeInitiative: (party) => computeInitiativeOrder(party),
}
